...
func TestRenderTemplate_RejectsTemplateInvocation(t *testing.T) {
	...
	if _, err := renderTemplate(body, renderTestSession(), &Action{}, nil, nil); err == nil {
		t.Fatal("expected nested template invocation to be rejected")
	}

	// Self-reference by the root template name is also blocked.
	if _, err := renderTemplate(`{{template "user" .}}`, renderTestSession(), &Action{}, nil, nil); err == nil {
...
func TestRenderTemplate_RejectsOversizeBody(t *testing.T) {
	...
	if _, err := renderTemplate(big, renderTestSession(), &Action{}, nil, nil); err == nil {
...
func TestRenderTemplate_DoesNotExposeMetadataOrHistory(t *testing.T) {
	...
	out, _ := renderTemplate(`[{{.Session.Metadata.api_key}}]`, renderTestSession(), &Action{}, nil, nil)
	if strings.Contains(out, "SECRET") {
		t.Fatalf("template leaked Session.Metadata: %q", out)
	}

	// Safe fields still resolve.
	out, err := renderTemplate(`{{.Vars.scene}}|{{.Session.PlatformID}}`, renderTestSession(), &Action{}, nil, nil)
	if err != nil {
